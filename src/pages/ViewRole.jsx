...
const ViewRole = ({ playerData, roomCode, phase, onNext, onLeave }) => {
        ...
        <div className="space-y-4 w-full">
          <button 
            ...
            className={`w-full py-5 rounded-2xl font-black uppercase tracking-widest transition-all active:scale-95 flex items-center justify-center gap-3 select-none
              ${isRevealed ? 'bg-slate-100 text-slate-950 shadow-inner' : isDead ? 'bg-slate-800 text-slate-500 border border-slate-700' : 'bg-red-700 hover:bg-red-600 shadow-lg'}`}
          >
          ...
          <div className="grid grid-cols-2 gap-3">
            <button onClick={() => setShowMechanics(true)} className="flex items-center justify-center gap-2 py-4 bg-slate-900 border border-slate-800 rounded-2xl text-amber-500 font-black text-[9px] uppercase tracking-widest">
