...
const ModeratorDashboard = ({ 
  ...
  const handleTimeSubmit = () => {
    ...
    if (parts.length === 2) {
      ...
      onEditTimer((minutes * 60) + seconds);
    }
    setIsEditing(false);
  };
        ...
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl shadow-2xl w-full lg:max-w-md space-y-6">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <button key={phase.id} onClick={() => onSetPhase(phase.id)} className={`flex flex-col items-center gap-1 p-3 rounded-xl border transition-all ${globalTimer.phase === phase.id ? `${phase.bg} border-${phase.color.split('-')[1]}-500/50 ${phase.color}` : 'border-slate-800 text-slate-600 hover:border-slate-700'}`}>
                <phase.icon size={18} />
                <span className="text-[8px] font-black uppercase">{phase.label}</span>
              </button>
            ))}
          </div>
          ...
          <div className="bg-slate-950/50 border border-slate-800 p-5 rounded-2xl relative overflow-hidden">
            <div className="flex items-center justify-between gap-4 relative z-10">
              ...
              <div className="flex-[1.8] flex justify-center scale-110"><SharedTimer roomCode={roomCode} /></div>
              <div className="flex-1 flex justify-center">
                <button onClick={() => onEditTimer(globalTimer.seconds + 60)} className="w-full aspect-square flex flex-col items-center justify-center bg-blue-600/10 text-blue-500 border border-blue-600/20 rounded-xl hover:bg-blue-600/20 transition-all group">
          ...
          <div className="flex gap-2">
            <button onClick={() => onToggleTimer(globalTimer.isActive, tempSeconds)} className={`flex-1 flex justify-center items-center gap-3 p-4 rounded-2xl font-black transition-all shadow-lg ${globalTimer.isActive ? 'bg-amber-600 hover:bg-amber-500' : 'bg-green-700 hover:bg-green-600'}`}>
